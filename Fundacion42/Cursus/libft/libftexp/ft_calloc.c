...
/*   Updated: 2023/02/26 18:56:46 by dmena-ro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
// Esta función reserva memoria para un array de nitems elementos de 
tamaño size cada uno y los inicializa a cero
#include "libft.h"
...
void	*ft_calloc(size_t nitems, size_t size)
{
    ...
    // Llamamos a la función malloc pasando el producto de nitems y 
	size como argumento y asignamos el resultado a block
	block = malloc(nitems * size);
	
    // Si block es NULL, significa que no se pudo reservar memoria 
	y devolvemos NULL como resultado de la función
	if (block == NULL)
		return (NULL);
    // Llamamos a la función ft_bzero pasando el puntero block y 
	el producto de nitems y size como argumentos para borrar la memoria reservada
	ft_bzero(block, (nitems * size));
